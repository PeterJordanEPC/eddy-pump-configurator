...
const checks = [
  ...
  [source.includes("Get my project quote") && source.includes("Get fast project pricing ↓") && source.includes("fast, engineering-reviewed project pricing") && !/instant (price|quote)/i.test(source), "quote CTAs are benefit-led without promising instant firm pricing"],
  ...
  [source.includes("font-size:16px; min-height:48px") && source.includes("min-width:20px; min-height:20px"), "form controls are sized for older users"],
  [source.includes("--orange: #C94F0A") && source.includes("grid-template-columns:112px minmax(0,1fr)"), "mobile cards are compact and orange contrast meets normal-text target"],
